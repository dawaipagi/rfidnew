						...
						<?php echo form_open_multipart($dataForm[$key_action]['url']); ?>
							<div class="box-body table-responsive">
								...
								if(isset($showSearch)){
									...
									echo "Dari hasil pencarian terdapat <b>".$decodeSS['result']."</b> data";
								}
								?>
							</div>
							<div class="box-footer">
								...
								if(isset($showSearch)){
									$decodeSS=json_decode($showSearch,true);
									echo '<button type="button" class="btn btn-success btn-sm" onclick=window.open("'.base_url().'report/print_pdf/'.jarvis_encode(json_encode($decodeSS['query'])).'","_blank")>Cetak PDF</button>';
								}
								?>
							</div>
						<?php echo form_close();?>
